...
def run(input_sizes,epochs,learning_rate,batch,imnet_path,architecture,resnet_weight_path,workers,shared_folder_path,job_id,local_rank,global_rank,num_tasks,EfficientNet_models):
    ...
    if Path(str(shared_folder_path)).is_dir():
        shared_folder=Path(shared_folder_path+"/finetune/1/")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--learning-rate', default=1e-2, type=float, help='base learning rate')
    parser.add_argument('--epochs', default=20, type=int, help='epochs')
    parser.add_argument('--input-size', default=320, type=int, help='images input size')
    parser.add_argument('--batch', default=32, type=int, help='Batch by GPU')
    parser.add_argument('--imnet-path', default='/home/hangnt/hangnt/FixRes-master/image/', type=str, help='Image Net dataset path')
    parser.add_argument('--architecture', default='EfficientNet', type=str,choices=['ResNet50', 'PNASNet' , 'IGAM_Resnext101_32x48d','EfficientNet'], help='Neural network architecture')
    parser.add_argument('--resnet-weight-path', default='/home/hangnt/hangnt/FixRes-master/weights/ResNet50_v2.pth', type=str, help='Neural network weights (only for ResNet50)')
    ...
    parser.add_argument('--num-tasks', default=1, type=int, help='How many GPUs are used')
    parser.add_argument('--shared-folder-path', default='/home/hangnt/hangnt/FixRes-master/save_folder2/', type=str, help='Shared Folder')
    parser.add_argument('--EfficientNet-models', default='tf_efficientnet_b7_ns', type=str, help='EfficientNet Models')
    ...
    #python main_finetune.py --input-size 320 --architecture 'EfficientNet' --epochs 50 --batch 2 --num-tasks 1 --EfficientNet-models 'tf_efficientnet_b7_ns'