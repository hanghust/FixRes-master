...
import pandas as pd
import pygal
logdir = "runs/FixRes/" + datetime.now().strftime("%Y%m%d-%H%M%S")
...
class Trainer:
    ...
    def _init_state(self) -> None:
        ...
        optimizer = optim.Adam(model.parameters(), lr=linear_scaled_lr, weight_decay=1e-4)
        # linear_scaled_lr = 8.0 * self._train_cfg.lr * self._train_cfg.batch_per_gpu * self._train_cfg.num_tasks / 512.0
        # optimizer = optim.SGD(model.par/ameters(), lr=linear_scaled_lr, momentum=0.9, weight_decay=1e-4)
    ...
    def _train(self) -> Optional[float]:
        ...
        print(cnf_matrix)
        print('\nAccuracy_confusion_matrix:', np.diagonal(cnf_matrix).sum()/cnf_matrix.sum())
        print(f"Accuracy of the network on the {total} test images: {acc:.1%}", flush=True)
        print(f"Loss of the network on the {total} test images: {ls_nm:.3f}", flush=True)
        ...
        for epoch in range(start_epoch, self._train_cfg.epochs):
            ...
            for i, data in enumerate(self._train_loader):
                ...
                if i % print_freq == print_freq - 1:
                    ...
                    count_y_train_T = OrderedDict(sorted(count_y_train_T.items()))
                    ...
                    writer.add_scalar("loss iter", running_loss/print_freq, j)
                    ...
                    writer.add_scalar("learning rate iter", self._state.lr_scheduler.get_last_lr()[0], j)
                    running_loss = 0.0
            ...
            if epoch==self._train_cfg.epochs-1:
                ...
                recall_micro = 100 * recall_score(y_true, y_pred, average='micro')
                print("Recall(micro): ", recall_micro)
                recall_macro = 100 * recall_score(y_true, y_pred, average='macro')
                ...
                acc = correct / total
                ls_nm=running_val_loss/count
                matrix_measure = [acc_, precision_micro, precision_macro, recall_micro, recall_macro, f1_micro, f1_macro]
                header = map(str, matrix_measure)
                import csv
                writer_ = csv.writer(open('accuracy_measure.csv', 'w+'))
                for word in header:
                    writer_.writerow([word,])
